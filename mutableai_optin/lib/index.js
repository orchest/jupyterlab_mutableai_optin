...
const SHOWN_OPTIN = "mutablai_optin:shown";

export default [
  {
    id: 'mutableai_optin',
    requires: [IMutableAI, IStateDB],
    autoStart: true,
    activate: function (app, mutableManager, state) {
      console.log(
        'JupyterLab extension mutableai_optin is activated!'
      );
      ...
      function initOptin() {
        ...
        let showOptIn = () => {
          ...
          p.then((r) => {
            if (r.button.accept) {
              mutableManager.enabled();
            } else {
              mutableManager.disable();
            }
            state.save(SHOWN_OPTIN, true);
          });
        }
      }

      state.fetch(SHOWN_OPTIN).then((shown) => {
        if (!shown) {
          initOptin();
        }
      })
